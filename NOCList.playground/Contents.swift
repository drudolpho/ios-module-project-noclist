...
let jimPhelps = (coverName: "Jim Phelps", realName: "John Voight", accessLevel: 9, compromised: true)
...
let maxRotGrab = (coverName: "Max RotGrab", realName: "Venessa Redgrave", accessLevel: 4, compromised: false)
...
var agentsArray = [ethanHunt, jimPhelps, clairePhelps, eugeneKittridge, franzKrieger, lutherStickell, sarahDavies, maxRotGrab, hannahWilliams, jackHarmon, frankBarnes]
...
func compromisedCounter() -> Int {
    var numberOfCompromisedAgents = 0
    for agent in agentsArray {
        if agent.compromised == true {
            numberOfCompromisedAgents += 1
        }
    }
    return numberOfCompromisedAgents
}

print(compromisedCounter())
//: ## Step 4
//: Call the above function to find the total number of compromised agents and then print a sentence that says "# agents have been compromised!" using string interpolation.

print("\(compromisedCounter()) agents have been compromised")
...
func findCleanAgents() -> [String] {
    var cleanAgentsArray: [String] = []
    for agent in agentsArray {
        if agent.compromised == false {
            print(agent.coverName)
            cleanAgentsArray.append(agent.coverName)
        }
    }
    return cleanAgentsArray
}
...
func levelTotaler() {
    ...
    print("\(lowLevelTotal) low level agents, \(midLevelTotal) mid level agents, and \(highLevelTotal) hight level agents")
}
...
//func ascendingOrder() {
//    var agentsInAscendingOrder = agentsArray.sorted(by: accessLevel)
//    print(agentsInAscendingOrder)
//}
